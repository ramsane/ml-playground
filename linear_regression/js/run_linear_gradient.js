...
function compute_Cost(w, x, y){
    ...
    let error = math.evaluate('y_pred - y',{y, y_pred})

    // no of training samples.
    let m = y.length
    return math.evaluate("(1/2)*((error*error')/m)",{error,m})
}
/***************************************************
...
function run_gradient_descent(x, y, w, iterations, learning_rate, is_stochastic, batch_size){
    ...
    function grad_descenent(){
        if(iter < iterations){
            ...
            let delta_w = math.evaluate("(X * (y_pred - sample_y)') * (1/m)",
                                    {X, y_pred, sample_y, m});
            w = math.evaluate("w - learning_rate*delta_w",{w, learning_rate, delta_w});
